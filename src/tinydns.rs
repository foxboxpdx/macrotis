...
// Define the TinyDNSRecord struct and implement some Traits on it
#[derive(Debug, Hash)]
...
impl PartialEq for TinyDNSRecord {
    // Consider a TDR to equal another one even if their TTLs differ
...
// For sorting and using Vec::dedup
...
impl std::fmt::Display for TinyDNSRecord {
	// Pretty print to make it look like a line in a BIND file
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{}\t{}\tIN\t{}\t{}", self.fqdn, self.ttl, self.rtype, self.target)
    }
}

// Little helper function that figures out what Zone from the config
// file a given TDR belongs to by taking the longest match between
// TDR.fqdn and Zone.domain
pub fn find_zone_id(tdr: &TinyDNSRecord, zones: &Vec<Zone>) -> Option<String> {
	let mut match_tuple = ("X".to_string(), None);
	for z in zones {
		if tdr.fqdn.contains(&z.domain) {
			if z.domain.len() > match_tuple.0.len() {
				match_tuple = (z.domain.to_string(), Some(z.id.to_string()));
			}
		}
	}
	// If there were any matches, then this will be Some(String);
	// otherwise it will still be None.
	match_tuple.1
}

// Given a filename, read in the contents, sending each line to the
// 'from_string' function to be turned into one or more TDRs.  Returns
// a Vec of TDRs on success or None on error.
pub fn from_file(fname: &str) -> Option<Vec<TinyDNSRecord>> {
    ...
    // Because of that, all the parse_X functions return a Vec that
    // can be simply append()-ed to retval, even if that Vec is empty.
...
// Process a string representing one line in a TinyDNS-formatted file.
...
// Also check to ensure each FQDN is all lowercase so we don't wind up
// with mixed-case duplicates as well.
fn check_dups(records: &Vec<TinyDNSRecord>) {
    let mut uniq = HashMap::new();
    for rec in records {
		if &rec.fqdn != &rec.fqdn.to_lowercase() {
			println!("Warning, mixed case detected: {}", &rec);
		}
